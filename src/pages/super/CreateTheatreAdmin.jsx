...
const Card = ({ children, className = "" }) => (
  <div className={`bg-white border border-slate-200 rounded-2xl shadow-sm p-6 ${className}`}>{children}</div>
);
...
export default function CreateTheatreAdmin() {
  ...
  const validate = () => {
    ...
    if (!form.email.trim() || !isValidEmail(form.email)) errs.email = "Enter a valid email.";
    if (!form.password || form.password.length < 6) errs.password = "Password must be ≥ 6 characters.";
    if (form.theatreId && !isValidObjectId(form.theatreId)) errs.theatreId = "Theatre ID must be a 24-char ObjectId or empty.";
    setErrors(errs);
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      console.error("create theatre admin error", e?.response || e);
      const code = e?.response?.data?.code;
      const message = e?.response?.data?.message || e?.message || "Failed to create";
      setMsg(`${code ? code + ": " : ""}${message}`);
      setMsgType("error");
    } finally {
      setLoading(false);
      // auto-clear info messages after a short delay
      if (msgType !== "error") setTimeout(() => setMsg(""), 3500);
    }
  };
    ...
    <main className="p-6">
      <div className="max-w-md mx-auto">
        <Card>
          ...
          <p className="text-sm text-slate-600 mb-4">Create an admin account scoped to a theatre (optional).</p>

          <form onSubmit={submit} className="space-y-3" aria-label="Create theatre admin form">
            <div>
              ...
              />
              {errors.name && <div id="err-name" className="text-xs text-rose-700 mt-1">{errors.name}</div>}
            </div>
            ...
            <div>
              ...
              />
              {errors.email && <div id="err-email" className="text-xs text-rose-700 mt-1">{errors.email}</div>}
            </div>
            ...
            <div>
              ...
              />
              {errors.password && <div id="err-password" className="text-xs text-rose-700 mt-1">{errors.password}</div>}
            </div>

            <div>
              <label className="block text-sm font-medium text-slate-700">Theatre ID (optional)</label>
              <input
                className={`w-full mt-1 px-3 py-2 rounded-xl text-sm border ${
                ...
                onChange={(e) => setForm((f) => ({ ...f, theatreId: e.target.value }))}
                aria-invalid={!!errors.theatreId}
                aria-describedby={errors.theatreId ? "err-theatreId" : undefined}
              />
              {errors.theatreId && <div id="err-theatreId" className="text-xs text-rose-700 mt-1">{errors.theatreId}</div>}
            </div>
