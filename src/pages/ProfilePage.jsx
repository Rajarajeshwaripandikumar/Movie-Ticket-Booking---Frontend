...
function SelectField({ label, value, onChange, options }) {
  return (
    <div>
      <div className="text-[12px] font-semibold uppercase text-slate-600 mb-1">{label}</div>
...
const Pill = ({ children, className = "" }) => (
  <span className={`inline-flex items-center rounded-full border border-slate-300 bg-slate-50 px-3 py-1 text-xs font-semibold text-slate-700 ${className}`}>
    {children}
  </span>
);
...
const Badge = ({ children, tone = "blue" }) => {
  ...
  return (
    <span className={`inline-flex items-center rounded-full px-3 py-1 text-xs font-bold border ${tones[tone] || tones.slate}`}>
      {children}
    </span>
  );
};
...
const IconUser = () => (<svg viewBox="0 0 24 24" className={ic} fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="7.5" r="3.5"/><path d="M4 20a8 8 0 0 1 16 0"/></svg>);
const IconPhone = () => (<svg viewBox="0 0 24 24" className={ic} fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M22 16.92v2a2 2 0 0 1-2.18 2 19.8 19.8 0 0 1-8.63-3.07A19.5 19.5 0 0 1 3.15 9.81 19.8 19.8 0 0 1 .08 1.18 2 2 0 0 1 2.06-.99h2a2 2 0 0 1 2 1.72c.12.9.33 1.78.61 2.62a2 2 0 0 1-.45 2.11L5.4 7.4a16 16 0 0 0 6.2 6.2l1.94-1.83a2 2 0 0 1 2.11-.45c.84.28 1.72.49 2.62.61a2 2 0 0 1 1.72 2v0"/></svg>);
const IconLock = () => (<svg viewBox="0 0 24 24" className={ic} fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><rect x="4" y="11" width="16" height="9" rx="2"/><path d="M8 11V8a4 4 0 0 1 8 0v3"/></svg>);
const IconTicket = () => (<svg viewBox="0 0 24 24" className={ic} fill="none" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round"><path d="M4 8.5V6a2 2 0 0 1 2-2h12a2 2 0 0 1 2 2v2.5a2.5 2.5 0 0 0 0 5V16a2 2 0 0 1-2 2H6a2 2 0 0 1-2-2v-2.5a2.5 2.5 0 0 0 0-5Z"/><path d="M9 6v12M15 6v12" opacity=".55"/></svg>);
const IconChevron = () => (<svg viewBox="0 0 20 20" className="w-4 h-4 text-slate-500" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M7.5 4.5L12 10l-4.5 5.5"/></svg>);
const IconArrow = () => (<svg viewBox="0 0 24 24" className="w-4 h-4" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M5 12h14"/><path d="M13 5l7 7-7 7"/></svg>);
...
export default function ProfilePage() {
  ...
  const [form, setForm] = useState({
    name: "",
    phone: "",
    preferences: { language: "en", notifications: { email: true, sms: false } },
  });
  ...
  useEffect(() => {
    ...
    (async () => {
        ...
        if (u0) {
          ...
          setUser(prev => {
            ...
            const same =
              ...
              prev.email === u.email &&
              prev.phone === u.phone &&
              JSON.stringify(prev.preferences || {}) === JSON.stringify(u.preferences || {});
            return same ? { ...prev, createdAt: u.createdAt || prev.createdAt } : u;
          });
      ...
      try {
        ...
        const d = r?.data ?? {};
        const list = Array.isArray(d) ? d : Array.isArray(d.bookings) ? d.bookings : [];
        setBookings(list);
      } catch (e) {
        if (ac.signal.aborted) return;
  ...
  useEffect(() => {
    ...
    setForm({
      ...
      phone: user.phone || "",
      preferences:
        user.preferences || { language: "en", notifications: { email: true, sms: false } },
    });
  ...
  async function handleSave(e) {
    ...
    try {
      ...
      if (updatedUser0) {
        ...
        const updatedUser = derived ? { ...updatedUser0, createdAt: derived } : updatedUser0;
        ...
        setForm({
          ...
          preferences:
            updatedUser.preferences || { language: "en", notifications: { email: true, sms: false } },
        });
      }
  ...
  async function handleChangePassword(e) {
    ...
    if (!pwCurrent || !pwNew) return setPwErr("Please fill current and new password.");
    if (pwNew.length < 8) return setPwErr("New password should be at least 8 characters.");
    if (pwNew !== pwConfirm) return setPwErr("New password and confirm do not match.");
    ...
    return (
      <div className="min-h-[60vh] w-full [margin-inline:calc(50%-50vw)] bg-slate-50 flex items-center justify-center px-6">
        <Card className="max-w-md w-full p-6 sm:p-8 text-center">
          ...
          <p className="text-sm text-slate-600 mt-2">You must be logged in to view this page.</p>
        </Card>
      </div>
    );
  }
    ...
    <div className="min-h-screen w-screen [margin-inline:calc(50%-50vw)] bg-slate-50 text-slate-900">
      <div className="max-w-5xl mx-auto px-4 md:px-6 py-6 md:py-10">
        ...
        <Card className="max-w-3xl mx-auto p-5 sm:p-6 mb-6">
          ...
          <div className="mt-4 grid grid-cols-[auto,1fr] gap-4 sm:gap-5 items-center">
            ...
            <div className="min-w-0">
              <div className="text-lg sm:text-xl font-bold truncate">{user?.name || "User"}</div>
              <div className="text-sm text-slate-600 truncate">{user?.phone || user?.email || "—"}</div>
              <div className="text-xs text-slate-500 mt-1">
                Member since <Badge tone="slate">{formatMemberSinceSafe(user)}</Badge>
              </div>
            </div>
          </div>
        </Card>
        ...
        <div className="max-w-3xl mx-auto grid gap-4">
          ...
          <Row
            ...
            helper={loadingBookings ? "Loading..." : bookings.length ? `${bookings.length} total` : "No bookings yet"}
            onClick={() => navigate("/bookings")}
          />

          <SectionLabel>Profile</SectionLabel>
          <Row icon={<IconUser />} label="Edit profile" onClick={() => setShowEdit(true)} />
          <Row icon={<IconLock />} label="Change password" onClick={() => setShowPassword(true)} />

          <SectionLabel>Account</SectionLabel>
          <Row icon={<span className="w-5 h-5 text-slate-600">↩</span>} label="Logout" onClick={logout} hideChevron />
        </div>
      </div>
          ...
          <form onSubmit={handleSave} className="space-y-3">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              ...
              <div className="space-y-2">
                <div className="text-[12px] font-semibold uppercase text-slate-600">Notifications</div>
                <Toggle
      ...
      {showPassword && (
        <Modal title="Change password" onClose={() => setShowPassword(false)}>
          {pwMsg && <Card className="mb-3 p-3 bg-emerald-50 border-emerald-200 text-emerald-700 font-semibold">{pwMsg}</Card>}
          {pwErr && <Card className="mb-3 p-3 bg-rose-50 border-rose-200 text-rose-700 font-semibold">{pwErr}</Card>}
          <form onSubmit={handleChangePassword} className="space-y-3" autoComplete="off">
            <Field type="password" placeholder="Current password" value={pwCurrent} onChange={setPwCurrent} />
            <Field type="password" placeholder="New password (min 8 chars)" value={pwNew} onChange={setPwNew} />
            <Field type="password" placeholder="Confirm new password" value={pwConfirm} onChange={setPwConfirm} />
      ...
      <Card className="w-full flex items-center justify-between px-4 py-3 hover:bg-slate-50 transition-colors">
        <div className="flex items-center gap-3">
          ...
          <div className="leading-tight">
            ...
            {helper ? <div className="text-[12px] text-slate-600 mt-0.5">{helper}</div> : null}
          </div>
        </div>
        {!hideChevron ? (
