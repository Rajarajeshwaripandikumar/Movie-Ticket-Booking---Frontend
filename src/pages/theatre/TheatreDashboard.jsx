...
const StatCard = ({ icon, value, label, loading = false }) => (
  <Card className="text-center">
    <div className="mx-auto h-6 w-6 text-[#0071DC]">{icon}</div>
    <div className="text-xl font-bold mt-2">
      {loading ? <span className="inline-block h-5 w-14 rounded bg-slate-200 animate-pulse" /> : value}
    </div>
...
const firstObj = (x) => (x && typeof x === "object" && !Array.isArray(x) ? x : null);
    ...
    async (signal) => {
      ...
      const bookings =
        typeof bData?.count === "number"
          ? bData.count
          : typeof bData?.totalBookings === "number"
          ? bData.totalBookings
          : Array.isArray(bData)
          ? bData.length
          : 0;
      ...
      <div className="max-w-5xl mx-auto px-4 space-y-5">
        <Card>
          ...
          <p className="text-sm text-slate-600 mt-1">
            {loading ? "Loading theatre..." : theatre ? `${theatre.name || "—"}${theatre.city ? " — " + theatre.city : ""}` : "Theatre not found"}
          </p>
        </Card>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <StatCard icon={<Monitor />} value={stats.screens ?? "—"} label="Screens" loading={loading} />
          <StatCard icon={<CalendarClock />} value={stats.upcomingShowtimes ?? "—"} label="Upcoming Showtimes" loading={loading} />
          <StatCard icon={<BarChart3 />} value={stats.bookings ?? "—"} label="Bookings (period)" loading={loading} />
        </div>

        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <Link to="/theatre/screens" aria-label="Manage screens">
            <Card className="text-center hover:shadow-md transition">Manage Screens</Card>
          </Link>
          <Link to="/theatre/showtimes" aria-label="Manage showtimes">
            <Card className="text-center hover:shadow-md transition">Manage Showtimes</Card>
          </Link>
          <Link to="/theatre/reports" aria-label="View reports">
            <Card className="text-center hover:shadow-md transition">Reports</Card>
          </Link>
        </div>
        ...
        {!loading && !theatre && !err && (
          <Card className="text-center">
            <p className="text-sm text-slate-600 mb-3">No theatre record found for your account.</p>
            <div className="flex items-center justify-center gap-3">
              <Link to="/theatre/create" className="inline-block rounded-full px-4 py-2 text-sm font-semibold bg-[#0071DC] text-white">
                Create Theatre
              </Link>
              <Link to="/support" className="text-sm text-[#0654BA] underline">
                Contact Support
              </Link>
            </div>
          </Card>
