...
// Full updated file: robust fetches, better logging, endpoint fallbacks,
// handles many backend response shapes and avoids uncaught promise rejections.
 ...
 * prefer /admin/theaters/:id/screens then /theaters/:id/screens then alias /screens/by-theatre/:id
 */
async function fetchScreensForTheater(theaterId) {
  const candidates = [
    ...
    // compatibility alias: returns raw array in older frontends
  ...
  for (const path of candidates) {
    try {
      ...
      console.debug(`[AdminScreens] fetchScreens tried ${path} -> status=${res.status} got=${Array.isArray(arr) ? arr.length : "not-array"}`);
      ...
      console.warn(`[AdminScreens] fetchScreens endpoint ${path} failed:`, { status: st, body });
      // try next
    }
  }
  ...
  useEffect(() => {
    (async () => {
      const candidates = [
        ...
        "/theatres/mine",
        "/theaters/admin/list",
        "/movies/admin/list", // sometimes frontends try movie aliases — harmless to check
      ];
      ...
      for (const p of candidates) {
        try {
          ...
          console.debug(`[AdminScreens] tried ${p} -> status=${res.status} preview=${JSON.stringify(res?.data).slice(0, 300)}`);
          ...
          const body = err?.response?.data ?? err.message ?? String(err);
          // eslint-disable-next-line no-console
          console.warn(`[AdminScreens] endpoint ${p} failed:`, { status: st, body });
          // surface auth-related messages
          if (st === 401 || st === 403) {
            setMsg("Authentication/authorization error while loading theaters. Ensure you have an admin token.");
            setMsgType("error");
            break;
          }
          if (st === 400) {
            setMsg(`Server rejected request to ${p} (400). Check console for details.`);
            setMsgType("error");
            // continue — some endpoints may return 400 while others work
          }
        }
      }
      ...
      if ((!Array.isArray(list) || list.length === 0) && lastErr) {
        // eslint-disable-next-line no-console
        console.debug("[AdminScreens] no theaters found after trying candidates; last error:", lastErr?.response?.data ?? lastErr?.message ?? lastErr);
      }
    })();
  }, []);
    ...
    try {
      if (selectedScreen === NEW) {
        ...
        const candidates = [
          ...
          `/theatres/${selectedTheater}/screens`,
        ];
        let ok = false;
        for (const p of candidates) {
          try {
        ...
        const candidates = [
          ...
          `/theatres/${selectedTheater}/screens/${selectedScreen}`,
        ];
        let ok = false;
        for (const p of candidates) {
          try {
        ...
        <Card className="p-5">
          <div className="flex items-center justify-between">
            ...
            <SecondaryBtn
              onClick={async () => {
                try {
                  // refresh theaters with fallback (same candidates as loader)
                  const candidates = [
                    "/admin/theaters",
                    "/admin/theatres",
                    "/api/admin/theaters",
                ...
                if (selectedTheater) {
                  try {
                    ...
                    // ignore; fetchScreens logs details
                  }
                }
              }}
            >
