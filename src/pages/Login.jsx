  ...
  async function handleForgotPassword(e) {
    ...
    try {
      ...
      if (error?.response?.status === 404) {
        setMsg("Password reset is not available right now. Please contact support or try again later.");
      } else {
        ...
        {!showForgot ? (
          <form onSubmit={handleLogin} className="space-y-4">
            ...
            <div className="text-sm flex justify-between items-center">
              ...
              <Link to="/register" className="text-sm font-semibold underline decoration-2 underline-offset-4 text-[#0654BA] hover:text-[#004A99]">
                Create account
              </Link>
            </div>
          </form>
