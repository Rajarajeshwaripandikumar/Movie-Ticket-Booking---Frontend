...
function normalizeCastItem(item) {
  ...
  if (item && typeof item === "object") {
    if (item.actorName || item.name || item.character) {
      return { actorName: item.actorName || item.name || "", character: item.character || undefined };
    }
...
function parseCast(anyCast) {
  ...
  if (typeof anyCast === "string") {
    ...
    if (!s) return [];
    try { return parseCast(JSON.parse(s)); }
    catch {
...
export default function MovieDetail() {
  ...
  useEffect(() => {
    ...
    (async () => {
      const found = new Set();
      try {
        const r = await api.get("/showtimes/cities", { params: { movieId: routeMovieId, date } });
        if (Array.isArray(r.data)) r.data.forEach((c) => found.add(c));
      } catch {}
      try {
        if (!found.size) {
          const r = await api.get("/theaters", { params: { limit: 1000 } });
          if (Array.isArray(r?.data?.theaters)) r.data.theaters.forEach((t) => t?.city && found.add(t.city));
        }
      } catch {}
      try {
        if (!found.size) {
          const r = await api.get("/showtimes", { params: { movieId: routeMovieId, date } });
          if (Array.isArray(r.data)) r.data.map((s) => s?.theater?.city).filter(Boolean).forEach((c) => found.add(c));
        }
      } catch {}
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        setErr("");
        const { data } = await api.get("/showtimes", { params: { movieId: routeMovieId, city, date } });
  ...
  const grouped = useMemo(() => {
    ...
    for (const s of showtimes) {
      const thId = s?.theater?._id || "unknown";
      if (!map.has(thId)) {
        map.set(thId, {
          id: thId,
    ...
    >
        ...
        <div className="px-6 md:px-8 py-6">
          {tab === "synopsis" && (
            <div className="space-y-5">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <div className="font-semibold text-slate-800">{typeof movie.runtime === "number" ? `${movie.runtime} mins` : "—"}</div>
                  </div>
                </div>
                <div className="flex items-start gap-3">
                  <IconCalendar />
              ...
              <div className="pt-2 flex gap-3 overflow-x-auto hide-scrollbar">
                {[movie.poster, movie.posterUrl, ...(movie.images || [])]
                  .filter(Boolean)
                  .slice(0, 6)
                  .map((src, i) => (
          ...
          {tab === "posters" && (
            <div className="space-y-4">
              ...
              {[movie.poster, movie.posterUrl, ...(movie.images || [])].filter(Boolean).length ? (
                ...
                {grouped.map((th) => (
                  <div key={th.id} className="text-sm">
                    ...
                    <div className="mt-1 flex flex-wrap gap-2">
                      {th.items.map((s) => {
                        const disabled = s.seatsAvailable === 0;
                        return (
                          <Link
                            key={s._id}
                            to={`/seats/${s._id}`}
      ...
      <div className="hidden md:block absolute left-6 bottom-6 pointer-events-none opacity-10">
        {(movie.poster || movie.posterUrl) && (
          <Card className="p-1">
            <img src={movie.poster || movie.posterUrl} alt="" className="w-40 h-60 object-cover rounded-xl" />
          </Card>
        )}
      </div>
    </div>
