// src/pages/AdminShowtimes.jsx — hardened admin showtimes with safer fetches, aborts, and clearer UX
...
async function tryFetchCandidates(candidates = []) {
  ...
  for (const rawEp of candidates) {
    ...
    try {
      ...
      if (API_DEBUG || (status && status >= 400)) console.warn(`[${status ?? "ERR"}] GET ${rawEp}`, e?.response?.data || e?.message || e);
    }
  }
  if (API_DEBUG && lastErr) console.warn("All candidates failed:", candidates, lastErr?.message || lastErr);
  return [];
}
...
export default function AdminShowtimes() {
  const auth = useAuth() || {};
  // prefer adminToken if present; AuthProvider controls axios too
  const token = auth.adminToken || auth.token || null;
  ...
  /* toast helper (auto-clear) */
  const showMsg = useCallback((text = "", type = "info", ms = 4500) => {
    if (!mountedRef.current) return;
    setMsg(text); setMsgType(type);
    if (toastRef.current) clearTimeout(toastRef.current);
    if (ms) toastRef.current = setTimeout(() => { if (mountedRef.current) { setMsg(""); setMsgType("info"); } }, ms);
  }, []);
  ...
  const loadMovies = useCallback(async () => {
    try {
      ...
      if (!mountedRef.current) return;
      setMovies(list || []);
      if (Array.isArray(list) && list.length && !movieId) setMovieId(list[0]._id || list[0].id || "");
    } catch (err) {
  ...
  const loadScreensForTheater = useCallback(async (id) => {
    if (!id) {
    ...
    try {
      ...
      if (!mountedRef.current) return;
      setScreens(list || []);
      if (Array.isArray(list) && list.length && !screenId) {
        const first = list[0];
  ...
  const loadShowtimesMyTheatre = useCallback(async (thId = theaterId) => {
    try {
      const candidates = ["/showtimes/my-theatre", thId ? `/showtimes?theaterId=${thId}` : null, "/showtimes"].filter(Boolean);
      const list = await tryFetchCandidates(candidates);
      if (!mountedRef.current) return;
      setShowtimes(list || []);
      if (Array.isArray(list) && list.length && showtimeId === NEW) {
  ...
  const createShowtime = useCallback(async (e) => {
    e?.preventDefault?.();
    ...
    try {
      ...
      for (const epRaw of createCandidates) {
        try {
          ...
          if (API_DEBUG || (status && status >= 400)) console.warn(`[${status ?? "ERR"}] POST ${epRaw}`, err?.response?.data || err?.message || err);
        }
      }
      if (!created) {
  ...
  }, [movieId, theaterId, screenId, city, startTime, basePrice, rows, cols, loadShowtimesMyTheatre, showMsg]);

  const patchShowtime = useCallback(async (e) => {
    e?.preventDefault?.();
    ...
    try {
      ...
      for (const epRaw of candidates) {
        try {
          ...
          lastErr = e;
          const status = e?.response?.status;
          if (API_DEBUG || (status && status >= 400)) console.warn(`[${status ?? "ERR"}] PATCH ${epRaw}`, e?.response?.data || e?.message || e);
        }
      }
      if (!patched) {
  ...
  }, [showtimeId, startTime, loadShowtimesMyTheatre, theaterId, showMsg]);

  const deleteShowtime = useCallback(async (id) => {
    if (!id) return;
    ...
    try {
      ...
      for (const epRaw of candidates) {
        try {
          ...
          lastErr = e;
          const status = e?.response?.status;
          if (API_DEBUG || (status && status >= 400)) console.warn(`[${status ?? "ERR"}] DELETE ${epRaw}`, e?.response?.data || e?.message || e);
        }
      }
      if (!deleted) {
  ...
  }, [loadShowtimesMyTheatre, theaterId, showMsg]);
  ...
  async function openSeatPreviewForScreen(screenIdArg) {
    ...
    try {
      ...
      if (Array.isArray(payload)) {
        // parse labels -> {row,col}
        const parsed = payload.map((lab) => {
        ...
        setSeatPreviewData({ seats: parsed, rows: parsed.length ? undefined : 0, cols: undefined, title: "Screen seats" });
        return;
      }
      if (payload?.data && Array.isArray(payload.data)) {
        setSeatPreviewData({ seats: payload.data, rows: payload.rows || undefined, cols: payload.cols || undefined, title: payload.name ? `${payload.name} seats` : "Screen seats" });
        return;
      }
      if (payload?.rows || payload?.cols) {
        ...
        const seatsArr = [];
        for (let rr = 1; rr <= r; rr++) for (let cc = 1; cc <= c; cc++) seatsArr.push({ row: rr, col: cc, status: "AVAILABLE" });
        setSeatPreviewData({ seats: seatsArr, rows: r, cols: c, title: `${payload.name || "Screen"} seats` });
        return;
      }
  ...
  /* keep city in sync when theater changes */
  useEffect(() => {
  ...
  useEffect(() => {
    // no-op if token missing
    if (!token) return;
    // load initial showtimes if theaters already present
    // (loadTheaters triggers loadShowtimesMyTheatre for first theatre)
    // we also expose a manual refresh via UI
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [token]);
                  ...
                  return (
                    ...
                    >
                      <div>
                        ...
                        <div className="text-[10px]">{String(status).replace("_", " ")}</div>
                      </div>
                    </div>
                  );
                })}
      ...
      <div className="w-full max-w-6xl mx-auto stack">
        <Card className="p-5 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <SecondaryBtn onClick={() => loadShowtimesMyTheatre()} disabled={loading}>
              <RefreshCcw className="h-4 w-4" /> Refresh
            </SecondaryBtn>
            <SecondaryBtn onClick={() => { loadMovies(); loadTheaters(); }} disabled={loading}>
              Reload Data
            </SecondaryBtn>
          </div>
        </Card>
        ...
        {msg && (
          <Card className={`p-3 font-semibold ${ msgType === "success" ? "bg-emerald-50 border-emerald-200 text-emerald-700" : msgType === "error" ? "bg-rose-50 border-rose-200 text-rose-700" : "bg-blue-50 border-blue-200 text-blue-700" }`}>
            {msg}
          </Card>
        )}
        ...
        <Card className="p-5">
          ...
          <form onSubmit={createShowtime} className="space-y-3">
            <Field as="select" value={movieId} onChange={(e) => setMovieId(e.target.value)} icon={Film}>
              ...
              {Array.isArray(movies) && movies.map((m) => <option key={m._id || m.id} value={m._id || m.id}>{m.title || m.name}</option>)}
            </Field>

            <Field as="select" value={theaterId} onChange={(e) => handleTheaterChange(e.target.value)} icon={Building2}>
              <option value="">-- Select Theater --</option>
              {Array.isArray(theaters) && theaters.map((t) => <option key={t._id || t.id} value={t._id || t.id}>{t.name} — {t.city || ""}</option>)}
            </Field>
              ...
              <div className="flex-1">
                <Field as="select" value={screenId} onChange={(e) => handleScreenChange(e.target.value)} icon={LayoutGrid} disabled={!theaterId}>
                  ...
                  {Array.isArray(screens) && screens.map((s) => {
                    ...
                    return <option key={s._id || s.id} value={s._id || s.id}>{s.name} {r && c ? `(${r}x${c})` : ""}</option>;
                  })}
                </Field>
              </div>
              <div className="w-40">
                <SecondaryBtn type="button" onClick={() => openSeatPreviewForScreen(screenId)} disabled={!screenId || loading}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              ...
              <Field type="number" placeholder="Base Price" value={basePrice} onChange={(e) => setBasePrice(e.target.value)} icon={CircleDollarSign} min="0" step="1" />
            </div>

            <div className="text-sm text-slate-600">Seat grid: {buildSeatHint(rows, cols) || "unknown — preview screen to see grid"}</div>

            <div className="flex items-center justify-end">
              <PrimaryBtn disabled={!canCreate || loading}>{loading ? "Creating…" : "Create Showtime"}</PrimaryBtn>
            </div>
          </form>
        </Card>
        ...
        <Card className="p-5">
          ...
          <form onSubmit={patchShowtime} className="space-y-3">
            <Field as="select" value={showtimeId} onChange={(e) => handleSelectShowtime(e.target.value)} icon={CalendarClockLucide}>
              ...
              {Array.isArray(showtimes) && showtimes.map((s) => (
                <option key={s._id || s.id} value={s._id || s.id}>
                  {(s.movie?.title || s.movie?.name || (s.movie && String(s.movie)) || "Movie")} — {s.city || ""} — {s.startTime ? new Date(s.startTime).toLocaleString() : s.startAt ? new Date(s.startAt).toLocaleString() : "—"} — ₹{s.basePrice ?? s.price ?? s.amount ?? ""}
                </option>
              ))}
            </Field>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              <Field type="datetime-local" value={startTime} onChange={(e) => setStartTime(e.target.value)} icon={CalendarClockLucide} />
              <div className="flex items-center gap-2">
                <SecondaryBtn type="button" onClick={() => openSeatPreviewForShowtime(showtimeId)} disabled={!showtimeId || showtimeId === NEW}> <Eye className="h-4 w-4" /> View Showtime Seats</SecondaryBtn>
                <div className="ml-auto">
                  <PrimaryBtn disabled={!canUpdate || loading}>{loading ? "Updating…" : "Update Showtime"}</PrimaryBtn>
                </div>
              </div>
            </div>
          </form>
                ...
                return (
                  <div key={s._id || s.id} className="flex items-center justify-between border border-slate-100 rounded-lg p-3">
                    <div>
                      ...
                      <div className="text-sm text-slate-600">{theatreName} • {s.screen?.name || s.screen || "screen"} • {start ? new Date(start).toLocaleString() : "—"}</div>
                    </div>

                    <div className="flex items-center gap-2">
                      <SecondaryBtn onClick={() => { handleSelectShowtime(s._id || s.id); window.scrollTo({ top: 0, behavior: "smooth" }); }}>Edit</SecondaryBtn>

                      <SecondaryBtn onClick={() => openSeatPreviewForShowtime(s._id || s.id)}><Eye className="h-4 w-4" /> Seats</SecondaryBtn>

                      <SecondaryBtn onClick={() => deleteShowtime(s._id || s.id)} className="text-rose-700 border-rose-200" disabled={loading}>Delete</SecondaryBtn>
                    </div>
                  </div>
                );
              })}
          ...
          <div className="bg-white rounded-2xl shadow-xl max-w-4xl w-[95%] p-4 relative">
            <div className="flex items-center justify-between mb-3">
              ...
              <div className="flex items-center gap-2">
                ...
                <button onClick={() => setSeatPreviewOpen(false)} className="inline-flex items-center justify-center p-2 rounded-full hover:bg-slate-100" aria-label="Close seat preview">
                  <X className="h-5 w-5" />
                </button>
              </div>
            </div>
...
/* --------------------------- SeatGrid (kept outside main to keep file tidy) --------------------------- */
function SeatGrid({ seats, rows: rCount, cols: cCount }) {
  const { map, maxRow, maxCol } = seatsToMap(seats);
  const rowsToRender = rCount || maxRow || 0;
  const colsToRender = cCount || maxCol || 0;
              ...
              {Array.from({ length: colsToRender }).map((_, cIdx) => {
                ...
                const classes =
                  status === "BOOKED"
                    ...
                    : "bg-emerald-50 border-emerald-200 text-emerald-800";
                return (
                  <div key={key} title={`${label} — ${status}`} className={`text-xs border rounded-md p-2 text-center ${classes} flex items-center justify-center`} style={{ minHeight: 36 }}>
                    <div>
                      <div className="font-semibold">{label}</div>
                      <div className="text-[10px]">{String(status).replace("_", " ")}</div>
                    </div>
