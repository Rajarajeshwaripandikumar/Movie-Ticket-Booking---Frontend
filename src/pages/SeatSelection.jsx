  ...
  return (
    <button
      ...
      className={`inline-flex w-full items-center justify-center gap-2 rounded-full px-5 py-3 font-semibold text-white 
        ${disabled ? "bg-slate-300 cursor-not-allowed" : "bg-[#0071DC] hover:bg-[#0654BA] focus-visible:ring-2 focus-visible:ring-[#0071DC]"} 
        ${className}`}
      {...props}
    >
      {children}
...
export default function SeatSelection() {
  ...
  const [screenSpec, setScreenSpec] = useState({ _id: "", name: "Screen", rows: 10, cols: 10 });
  ...
  const fetchShowtime = useCallback(async () => {
    try {
      ...
      const { data } = await api.get(`/showtimes/${showtimeId}?ts=${Date.now()}`);
      ...
      const popScreen = normalizeScreen(typeof st?.screen === "object" ? st.screen : null);
  ...
  // Poll more frequently to reduce staleness
  useEffect(() => {
  ...
  const toggleSeat = async (seat) => {
    ...
    const sInGrid = seats.find((s) => s.row === seat.row && s.col === seat.col);
    if (!sInGrid || sInGrid.status === "BOOKED") {
      pending.current.delete(key);
      return;
    }

    const alreadySelected = selected.some((s) => s.row === seat.row && s.col === seat.col);
    ...
    try {
      ...
      if (alreadySelected) {
        // optimistic remove
        setSelected((prev) => prev.filter((s) => !(s.row === seat.row && s.col === seat.col)));
        try {
  ...
  const secondsLeft = (() => {
    ...
    return Math.max(0, Math.floor((lockExpiresAt - serverNowApprox) / 1000));
  })();
  ...
  const proceedToCheckout = () => {
    ...
    if (!lockExpiresAt || lockExpired) return setMsg("⏳ Your seat hold expired. Please reselect.");
    ...
    <div className="min-h-screen bg-slate-50 py-8 px-4">
      <Card className="max-w-5xl mx-auto p-6 md:p-7">
        ...
        <div className="text-center mb-6">
          <h1 className="text-2xl font-extrabold text-slate-900">{showtime?.movie?.title || "Movie"}</h1>
          <p className="text-sm text-slate-600 mt-1">
            {(screenSpec.name || showtime?.screen?.name || "Screen")} • {showtime?.city} •{" "}
            {new Date(showtime?.startTime || showtime?.startAt).toLocaleString("en-IN")}
          </p>
          ...
          <div className="mt-3">
            ...
            {lockExpired && <span className="ml-3 text-sm text-rose-700">Seat hold expired.</span>}
          </div>
        </div>
        ...
        <Card className="border-none p-0 overflow-hidden">
          ...
          <div className="px-5 py-5">
            ...
            <div className="mt-6 flex items-center justify-center gap-6 text-sm">
              ...
              <div className="flex items-center gap-2">
                <span className="w-4 h-4 rounded-full inline-block" style={{ background: BLUE }} />
        ...
        <div className="mt-8">
          <p className="font-medium mb-1 text-center">
            Selected Seats:{" "}
            {selected.length ? selected.map((s) => `[${rowLabel(s.row)}${s.col}]`).join(", ") : "None"}
          </p>
          ...
          >
            {loading ? "Processing…" : lockExpired ? "Hold Expired" : "Proceed to Payment"}
          </PrimaryBtn>
        </div>
      </Card>
    </div>
