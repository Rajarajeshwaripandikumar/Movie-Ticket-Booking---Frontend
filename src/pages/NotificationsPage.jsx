...
export default function NotificationsPage() {
  ...
  const isTheatreAdmin = ["THEATRE_ADMIN", "THEATER_ADMIN", "ROLE_THEATRE_ADMIN"].includes(rawRole);

  const theatreId = useMemo(() => user?.theatreId || user?.theatre?._id || user?.theatre || user?.theaterId || user?.theater || null, [user]);
  ...
  const fetchAll = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const res = await api.get(buildUrl(), { headers: { Authorization: `Bearer ${token}` } });
  ...
  const markAll = async () => {
    ...
    setBusyAll(true);
    try {
      await api.post("/notifications/read-all", { theatreId: theatreId || undefined }, { headers: { Authorization: `Bearer ${token}` } });
  ...
  const getBookingId = (n) => n?.booking?._id || n?.data?.booking?._id || n?.data?.bookingId || n?.bookingId || null;
  const getShowtimeId = (n) => n?.showtime?._id || n?.data?.showtime?._id || n?.data?.showtimeId || n?.showtimeId || null;
  ...
  const openDetailsSafe = async (n) => {
    ...
    if (bookingId && t.includes("BOOKING")) {
      if (isObjectId(bookingId)) {
        navigate(isSuperAdmin || isTheatreAdmin ? `/admin/bookings/${bookingId}` : `/bookings/${bookingId}`);
        return;
      }
      try {
        ...
        if (resolved && resolved._id) {
          navigate(isSuperAdmin || isTheatreAdmin ? `/admin/bookings/${resolved._id}` : `/bookings/${resolved._id}`);
          return;
        }
      } catch {}
      if (isSuperAdmin || isTheatreAdmin) navigate(`/admin/bookings?q=${encodeURIComponent(bookingId)}`);
      else navigate(`/bookings?q=${encodeURIComponent(bookingId)}`);
      return;
    }

    if ((showtimeId && isObjectId(showtimeId)) || t.includes("SHOWTIME")) {
      const id = showtimeId;
      navigate(isSuperAdmin ? `/admin/showtimes/${id}` : isTheatreAdmin ? `/theatre/showtimes/${id}` : `/showtimes/${id}`);
      return;
    }
    ...
    <div className="p-6 max-w-6xl mx-auto">
      ...
      {loading ? (
        <div className="p-6 text-center text-slate-500">
          <div className="animate-spin h-8 w-8 border-4 border-t-transparent border-blue-500 rounded-full mx-auto"></div>
          <span>Loading notifications...</span>
        </div>
      ) : visible.length === 0 ? (
        <div className="p-8 border rounded text-center text-slate-500">
          <Inbox className="mx-auto mb-2 h-6 w-6" /> No notifications yet. Check back later!
        </div>
      ) : (
