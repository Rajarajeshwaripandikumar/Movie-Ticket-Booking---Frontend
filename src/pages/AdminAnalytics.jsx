...
const toRevenueDaily = (arr = []) =>
  (arr || []).map((d, i) => {
    const iso =
      d?.date?.slice?.(0, 10) || d?.dayISO || d?.day || d?._id || `D${i + 1}`;
...
export default function AdminAnalyticsDashboard() {
  ...
  async function loadData(selectedRange, providedSignal) {
    ...
    try {
      ...
      const revData = toRevenueDaily(revTrends || []);
      console.log("Revenue trends sample:", revTrends?.[0]);
      console.log("Transformed sample:", revData?.[0]);

      setRevenueDaily(revData);
    ...
    const makeCSV = (title, headers, rows) => {
      ...
      csv += (rows || [])
        .map((r) =>
          headers
            .map((h) => {
              if (h === "day") return csvEscape(r.dayISO ?? r.day ?? "");
          ...
          <Card className="p-4">
            <div className="flex items-center justify-between gap-3 flex-wrap">
              ...
              <div className="flex items-center gap-2 flex-wrap justify-end">
                <Pill onClick={() => { setAlertsOpen(true); refreshAlerts(); }}>
